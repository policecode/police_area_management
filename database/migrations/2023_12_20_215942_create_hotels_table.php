...
class CreateHotelsTable extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('hotels', function (Blueprint $table) {
            ...
            $table->string('slug');
            $table->string('address');// Địa chỉ kinh doanh
            $table->tinyInteger('type');// Loại hình kinh doanh
            $table->integer('emterprises_id')->nullable(); // Doanh nghiệp của cơ sở kinh doanh
            $table->text('manager');// Thông tin mô tả của người quản lý, lưu dạng json
            $table->text('note');// Thông tin mô tả về cơ sở kinh doanh.
            $table->timestamps();
        });
    }
