...
function jtrigger_error(message, style) {
    if (style != "" || style !== undefined) {
        ...
        style =
            "position:fixed;bottom:30px;left:30px;background:black;color:white;padding:10px;font-weight:bold;";
    }
    var html = jQuery(
        '<span class="error-message-alert" style="' +
            style +
            '">' +
            message +
            "</span>"
...
(function ($) {
            ...
            $.confirm.options,
            {
                confirm: function () {
                    ...
                    if (url) {
                        if (options.post) {
                            var form = $(
                                '<form method="post" class="hide" action="' +
                                    url +
                                    '"></form>'
                            );
                            $("body").append(form);
    ...
    jAlert = function (message, title, type) {
        ...
        <div class="toast-header">
            ...
            <button type="button" class="btn-close" onclick="document.querySelector('#fvn-popup-error').remove()"></button>
        </div>
    ...
    jAlertFocus = function (message, title, link, btn_text) {
        ...
        var button_text = "OK";
        if (btn_text != "" || btn_text != undefined) button_text = btn_text;

        if (title == "warning")
            title =
                '<span style="color:red"><i class="icon-warning"></i></span>&nbsp; Warning';
        var modalHTML =
...
function display_processing_form(enable) {
    if (enable) {
        jQuery("body").append(
            '<img id="jbform_loading"  style="position: fixed;top:50%;left: 50%;margin-left: -100px;margin-top: -100px;width:200px;height:200px;" src="' +
                FVN_PLUGIN_URL +
...
(function ($) {
    ...
    wfslideshow.prototype = {
        ...
        init: function () {
            var html =
                '<div class="slideshow_button slideshow_previous slideshow_transparent wf_slide_show_btn" route="pre" role="button" style="display:none;"></div><div class="slideshow_button slideshow_next slideshow_transparent" route="next" role="button"></div><div class="wf-slideshow-content">';

            var phone_class =
                "col-xs-" + Math.floor(12 / this.options.itemPhone);
            var tablet_class =
                "col-md-" + Math.floor(12 / this.options.itemTablet);
            var desktop_class =
                "col-lg-" + Math.floor(12 / this.options.itemDesktop);
            this.itemClass =
                phone_class + " " + tablet_class + " " + desktop_class;
            ...
            defaultHtml =
                ...
                '">' +
                '<div class="' +
                this.options.classAvatar +
                '"></div>' +
                '<div class="' +
                this.options.classTitle +
                '"></div>' +
                '<div class="' +
                this.options.classDesc +
                '"></div>' +
                "</div>";
            ...
            if (this.options.data.length == 0) {
                $.ajax({
                    ...
                    data: {
                        limit: wfslideshow.limit,
                        offset: wfslideshow.offset,
                    },
                    dataType: "json",
                    beforeSend: function () {
                        wfslideshow.$element
                            .find(".wf-slideshow-content")
                            .css("opacity", "0.5");
                    },
            ...
            $.each(data, function (i, item) {
                ...
                if (options.classMore != "") {
                    html +=
                        ...
                        '"><a href="' +
                        item.link +
                        '">Tìm hiểu thêm &nbsp;&nbsp;&nbsp;&nbsp;<i class="glyphicon glyphicon-play-circle"></i></a></div>';
                }
                html += "</div></div>";
            });
    ...
    $.fn.wfslideshow = function (option) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                $this.data(
                    "wfslideshow",
                    (data = new wfslideshow(this, options))
                );
            }
            if (
...
function jnotice(msg) {
    var html = `<div id="fvn-popup-success" class="toast text-bg-success position-fixed bottom-0 end-0 border-0 show" style="z-index: 9999;">
		<div class="d-flex align-items-center justify-content-between">
			...
			<button type="button" onclick="document.querySelector('#fvn-popup-success').remove()" class="btn-close btn-close-white me-2"></button>
		</div>
	</div>`;
    setTimeout(() => {
        if (document.querySelector("#fvn-popup-success")) {
            document.querySelector("#fvn-popup-success").remove();
        }
    }, 2000);
    document.querySelector("body").insertAdjacentHTML("beforeend", html);
}
...
const LocalStorageHelper = {
    ...
    __setArrayKeyToObject(treeKey, value, beforeValue) {
        ...
        if (treeKey.length === 1) {
            ...
            treeKey.shift();
            beforeValue[key1] = this.__setArrayKeyToObject(
                treeKey,
                value,
                beforeValue[key1]
            );
...
function getStringAfterTime(after_minutes, local = "en") {
    if (local == "en") {
        if (after_minutes < 60) {
            return after_minutes + " minutes ago";
        } else if (after_minutes < 60 * 24) {
            return Math.floor(after_minutes / 60) + " hours ago";
        } else if (after_minutes < 60 * 24 * 30) {
            return Math.floor(after_minutes / (60 * 24)) + " days ago";
        } else if (after_minutes < 60 * 24 * 30 * 365) {
            return Math.floor(after_minutes / (60 * 24 * 30)) + " months ago";
        } else {
            return (
                Math.floor(after_minutes / (60 * 24 * 30 * 365)) + " years ago"
            );
        }
    } else if (local == "vi") {
        if (after_minutes < 60) {
            ...
            return (
                Math.floor(after_minutes / (60 * 24 * 30 * 365)) + " năm trước"
            );
        }
    }
}
