var vue_data = {
    ...
    apiUrl: FVN_LARAVEL_HOME + '/admin/chapers-audio',
    story: story,
...
var app = new Vue({
    ...
    methods: {
        ...
        async save(e) {
            if (!confirm('Bạn có chắc chắn muốn tạo audio hàng loạt?')) {
                return;
            }
            e.preventDefault()
            this.loading = true;
            let jsonData = await new RouteApi().post(`${this.apiUrl}/${this.story.id}`,this.itemDetail );
            this.loading = false;
            if (jsonData.status) {
