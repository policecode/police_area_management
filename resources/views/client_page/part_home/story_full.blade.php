                ...
                <div class="section-stories-full__list">
                    @foreach ($full_stories as $item)
                        <div class="story-item-full text-center">
                            <a href="{{ route('client.story', ['story_slug' =>  $item['slug']]) }}" class="d-block story-item-full__image">
                                <img src="{{ $item['thumbnail'] }}" alt="Tự Cẩm"
                                    class="img-fluid w-100" width="150" height="230" loading="lazy">
                            </a>
                            <h3 class="fs-6 story-item-full__name fw-bold text-center mb-0">
                                <a href="{{ route('client.story', ['story_slug' =>  $item['slug']]) }}" class="text-decoration-none text-one-row story-name">
                                    {{ $item['title'] }}
                                </a>
                            </h3>
                            <span class="story-item-full__badge badge text-bg-success">Full - {{ $item['total_chaper'] }} chương</span>
                        </div>
                    @endforeach
