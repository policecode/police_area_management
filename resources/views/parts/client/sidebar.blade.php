            ...
            <div class="container">
                ...
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        ...
                        <li class="nav-item dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-custom">
                                @foreach ($all_categories as $key => $cat)
                                    <li><a class="dropdown-item" href="{{route('client.tag', ['tag_slug' => $cat['slug']])}}">{{$cat['name']}}</a>
                                    </li>
                                @endforeach
                    ...
                    <div class="d-flex header__form-search">
                        <input v-model="querySearch.keyword" class="form-control search-story" type="text" placeholder="Tìm kiếm" name="key_word">
                                ...
                                <div class="card-body p-0">
                                    <ul v-if="items.length > 0" class="list-group list-group-flush" >
                                        <li v-for="(item, index) in items" class="list-group-item hover-darkgrey">
                                            <a :href="item.url" :title="titleAuthor(item.author_name)" class="text-dark hover-title d-block">@{{item.title}}</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div v-if="(items.length == 0) && (querySearch.keyword.length > 3)" class="col-12 search-result shadow no-result"><div class="card text-white bg-light">
                            <div class="card-body p-0 text-dark text-align">Không tìm thấy truyện nào</div></div></div>
